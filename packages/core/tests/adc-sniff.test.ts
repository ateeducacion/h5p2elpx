...
describe("sniffAdcZip", () => {
  it("recognises native ADC bundles", async () => {
  ...
  it("recognises ADC plain zip bundles", async () => {
    const bytes = new Uint8Array(await readFile(resolve(FIXTURES, "sa1-zip.zip")));
    const r = await sniffAdcZip(bytes);
    expect(r).not.toBeNull();
    expect(r!.flavor).toBe("zip");
    expect(r!.hasPlainJson).toBe(true);
  });

  it("recognises ADC SCORM 1.2 bundles", async () => {
    const bytes = new Uint8Array(await readFile(resolve(FIXTURES, "sa1-scorm12.zip")));
    const r = await sniffAdcZip(bytes);
    expect(r).not.toBeNull();
    expect(r!.flavor).toBe("scorm12");
  });
