/**
 * Top-of-page utility bar.
 *
 * Adapted from the Claude Design handoff for h5p2elpx (chats/chat1.md
 * dated 2026-05-23): a quiet topbar with the eXeLearning brand on the
 * left and a GitHub link on the right. The brand mark uses the official
 * eXeLearning "eX" logo from doc/logo-only.svg in the upstream
 * exelearning/exelearning repo (white path on a teal square), not the
 * "eX" text placeholder from the design.
 */
const TEAL = "#078e8e";
const FG_INK = "#000000";
...
function ExeLogo() {
  // doc/logo-only.svg from the eXeLearning repo, white path so it sits
  // cleanly on the teal background.
  return (
    <svg
      viewBox="60 0 230 224"
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ color: FG_INK, fontWeight: 600 }}>h5p2elpx</span>
        <span style={{ color: "#999" }}>·</span>
        <span>H5P → eXeLearning converter</span>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 18, fontSize: "0.85rem" }}>
        <a
          href={githubUrl}
          target="_blank"
          ...
          onMouseEnter={(e) => {
            (e.currentTarget as HTMLAnchorElement).style.color = FG_INK;
          }}
          onMouseLeave={(e) => {
            (e.currentTarget as HTMLAnchorElement).style.color = FG_MUTED;
          }}
        >
          <GithubIcon />
          View on GitHub
